...
def hyperbolic_distance(u:th.Tensor, v:th.Tensor):
    ...
    c_angular_distance = dot / (r_u * r_v)
    #angular_distance = th.min(2*th.pi-th.abs(u[:,1]-v[:,1]), th.abs(u[:,1]-v[:,1]))

    d = th.arccosh(th.cosh(r_u)*th.cosh(r_v) - th.sinh(r_u)*th.sinh(r_v)*c_angular_distance)
    return d

def hyperbolic_distance_matrix(u:th.Tensor, v:th.Tensor):
    """ Calculates hyperbolic distance matrix between lists of positions.
    Args:
    ...
    r_u = th.norm(u, dim=-1)
    r_v = th.norm(v, dim=-1)
    # get the matrix containing the multiplications of r_u*r_v for each pair
    r_uv = th.mm(r_u.view(-1, 1), r_v.view(1, -1))

    # get the dot product of all pairs
    dot = th.mm(u, v.t())
    c_angular_distance = th.div(dot, r_uv)
    #angular_distance = th.min(2*th.pi-th.abs(u[:,1]-v[:,1]), th.abs(u[:,1]-v[:,1]))

    #d = th.arccosh(th.cosh(r_u)*th.cosh(r_v) - th.sinh(r_u)*th.sinh(r_v)*c_angular_distance)
    d = th.arccosh(th.mm(th.cosh(r_u).view(-1, 1), th.cosh(r_v).view(1, -1)) - th.mm(th.sinh(r_u).view(-1, 1), th.sinh(r_v).view(1, -1))*c_angular_distance)
    return d
